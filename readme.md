...
│  Repositories  │  AppDbContext  │  HttpClients   │
└─────────────────────────────────────────────────┘
│
┌─────────────────────────────────────────────────┐
│                  Domain Layer                    │
...
┌──────────────┐          ┌───────────────────┐          ┌─────────────────────────┐
│    USERS     │          │  SATELLITE_IMAGES  │          │  FIRE_DETECTION_RESULTS  │
│──────────────│  1     N │───────────────────│  1     1 │─────────────────────────│
│ Id (PK)      │─────────▶│ Id (PK)            │─────────▶│ Id (PK)                  │
│ Name         │          │ ImageUrl           │          │ FireDetected             │
│ Email        │          │ Region             │          │ RiskLevel                │
│ PasswordHash │          │ Latitude           │          │ ConfidenceScore          │
│ Role         │          │ Longitude          │          │ Notes                    │
│ CreatedAt    │          │ CapturedAt         │          │ AnalyzedAt               │
└──────────────┘          │ SubmittedAt        │          │ SatelliteImageId (FK)    │
│ UserId (FK)        │          └─────────────────────────┘
└───────────────────┘

### Diagrama de Fluxo — Pipeline de Detecção
Cliente
│
│  POST /api/satelliteimages
│  { region, latitude, longitude, capturedAt }
▼
API .NET ──── Salva imagem no Oracle ────▶ [201 Created] ──▶ Cliente
│
│ (background Task.Run)
▼
NASA GIBS
│  GET snapshot?BBOX=...&TIME=...
│  ◀── JPEG 512×512 ──
▼
Flask (Azure)
│  POST /api/v1/predict
│  multipart: imagem=satellite.jpg
│  ◀── { fogo_detectado, confianca_percentual } ──
▼
API .NET ──── Salva FireDetectionResult no Oracle

(disponível via GET /api/firedetectionresults/image/{id})
...
│
▼
{ token, name, email, role, expiresAt }

Rotas protegidas:
Authorization: Bearer {token}
│
▼
JwtBearerMiddleware valida
│
┌─────┴─────┐
▼           ▼
401       Acesso liberado
